...
#define N 1000000  
int *arr_asc;  // Mảng cho sắp xếp tăng dần
int *arr_desc; // Mảng cho sắp xếp giảm dần
int thread_count;
...
typedef struct {
    ...
    int right;
    int ascending;
    int *arr;  // Con trỏ đến mảng cần sắp xếp
} ThreadArgs;
...
int main() {
    srand(time(NULL));
    int *arr_orig = (int*)malloc(N * sizeof(int)); // Mảng gốc
    arr_asc = (int*)malloc(N * sizeof(int));       // Mảng tăng dần
    arr_desc = (int*)malloc(N * sizeof(int));      // Mảng giảm dần
    ...
    for (int t = 0; t < 5; t++) {
        ...
        /*
        printf("Ascending first 5: ");
        for(int j = 0; j < 5; j++) printf("%d ", arr_asc[j]);
        printf("\nDescending first 5: ");
        for(int j = 0; j < 5; j++) printf("%d ", arr_desc[j]);
        printf("\n");
        */
    }
